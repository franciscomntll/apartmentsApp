    ...
    return (
        ...
        <div className="relative w-full"   >
           ...
           {field?.value && <p className="text-sm text-gray-900">{field.value}</p>}
              ...
              <p htmlFor={field.name} className={`absolute left-2 -top-2 text-gray-400 text-xs transition-all px-1 bg-white cursor-pointer
              ...
              {state && <Dropdown set={act => handleClick(act) } options={options} />}
        {meta.touched && meta.error ? (
         <div className="error text-red-500 text-xs absolute pl-1 pt-0.5 bottom-0 left-0 transform translate-y-full">{meta.error}</div>
       ) : null}
        </div>
        </>
        </ClickAwayListener >
    ...
    return (
        <ul className="w-full absolute bottom-0 translate-y-full transform left-0 grid bg-white rounded-md shadow border z-30 text-sm cursor-pointer">
            {options?.map((item,idx) => (
                <li key={idx} onClick={() => set(item.title)} className="py-1 px-2 hover:bg-gray-200 transition">{item.title}</li>
            ))}
        </ul>
